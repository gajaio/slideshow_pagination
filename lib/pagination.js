...
(function($){
	...
	$.fn.pagination = function(params){
		...
		return this.each(function(){
			...
			var items = [];
            console.log($(this));
            if(self.options.width!=null) $(this).width(self.options.width);
            if(self.options.height!=null) $(this).height(self.options.height);
			...
			$(children).each(function(index){
				items[i] = (items[i] || "") + "<div style='float: left; position: relative;' id='item"+(index+1)+"' class='"+$(this).attr('class')+" item'>"+$(this).html()+"</div>";
				c_count--;
				if(c_count == 0) {
